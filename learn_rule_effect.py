...
class Logic_Model(nn.Module): 
    ...
    def optimize_EM_single(self, body_time, head_time, batch_size, head_predicate_idx, optimizer, pi, tau, num_samples):
        ...
        self.relation["1"]["[3, 4]"] = F.normalize(torch.ones(4)*torch.tensor([1000,1,1,1]),p=1,dim=0)
        self.relation["1"]["[3, 10]"] = F.normalize(torch.ones(4)*torch.tensor([1,1,1,1000]),p=1,dim=0)
        self.relation["1"]["[4, 10]"] = F.normalize(torch.ones(4)*torch.tensor([1,1,1,1000]),p=1,dim=0)
